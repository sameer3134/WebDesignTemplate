import React, { useState } from 'react'
import { FaHamburger } from 'react-icons/fa';
const Na = () => {
    let Links =[
      {name:"HOME",link:"/"},
      {name:"SERVICE",link:"/"},
      {name:"ABOUT",link:"/"},
      {name:"BLOG'S",link:"/"},
      {name:"CONTACT",link:"/"},
    ];
    let [open,setOpen]=useState(false);
  return (
    <div className='shadow-md w-full fixed top-0 left-0'>
      <div className='md:flex items-center justify-between bg-white py-4 md:px-10 px-7'>
      <div className='font-bold text-2xl cursor-pointer flex items-center font-[Poppins] 
      text-gray-800'>
        <span className='text-3xl text-indigo-600 mr-1 pt-1'>
        <img class="w-14" src="lo.png"></img>
        </span>
        Digcy
      </div>
      
      <div onClick={()=>setOpen(!open)} className='text-3xl absolute right-8 top-6 cursor-pointer md:hidden'>
      <FaHamburger name={open ? 'close':'menu'}></FaHamburger>
      </div>

      <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-0 transition-all duration-500 ease-in ${open ? 'top-20 ':'top-[-490px]'}`}>
        {
          Links.map((link)=>(
            <li key={link.name} className='md:ml-8 text-xl md:my-0 my-7'>
              <a href={link.link} className='text-gray-800 hover:text-gray-400 duration-500'>{link.name}</a>
            </li>
          ))
        }
        <button className='bg-white text-green-500 border border-green-500 font-bold py-2 px-6 rounded md:ml-6 hover:bg-green-500 hover:text-white 
    duration-500'>
          Log in 
        </button> &nbsp; &nbsp;
        <button className='bg-white text-green-500 border sm:ml-10 border-green-500 font-bold py-2 px-6 rounded md:ml-4 hover:bg-green-500 hover:text-white 
    duration-500'>
          Sign up
        </button>
      </ul>
      </div>
    </div>
  )
}

export default Na